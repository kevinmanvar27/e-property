    ...
    <div class="ps-3">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb mb-0 p-0">
                ...
                <li class="breadcrumb-item"><a href="{{ route('plot.index') }}">Plot List</a></li>
...
<div class="card">
    <div class="card-header">
        <h5 class="mb-0">Edit Plot Record</h5>
    </div>
    <div class="card-body">
        <form id="plot-form" enctype="multipart/form-data">
            @csrf
            @method('PUT')
                    ...
                    <div class="col-md-12">
                        ...
                        <div class="d-flex flex-wrap" id="amenities-container">
                            ...
                            <div class="form-check me-3 mb-2">
                                <input class="form-check-input" type="checkbox" name="amenities[]" value="{{ $amenity->id }}" id="amenity_{{ $amenity->id }}" {{ in_array($amenity->id, old('amenities', $selectedAmenities)) ? 'checked' : '' }}>
            ...
            <div class="d-flex justify-content-end">
                ...
                <button type="submit" class="btn btn-primary">Update Plot Record</button>
            </div>
        </form>
    </div>
</div>
...
document.getElementById('plot-form').addEventListener('submit', function(e) {
    ...
    fetch("{{ route('plot.update', $property->id) }}", {
        method: 'POST',
        body: formData,
        headers: {
    ...
    .catch(error => {
        ...
        const generalError = document.createElement('div');
        generalError.className = 'alert alert-danger';
        generalError.textContent = 'An error occurred while updating the plot record. Please check that your files are under 100MB and try again.';
        generalError.style.marginBottom = '20px';
...
function deletePhoto(photoIndex) {
    if (confirm('Are you sure you want to delete this photo?')) {
        fetch(`/admin/plot/{{ $property->id }}/photos/${photoIndex}`, {
            method: 'DELETE',
            headers: {
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
            }
