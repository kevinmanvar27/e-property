    ...
    <div class="ps-3">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb mb-0 p-0">
                ...
                <li class="breadcrumb-item active" aria-current="page">Shad List</li>
            </ol>
        </nav>
    </div>
    <div class="ms-auto">
        <div class="btn-group">
            <a href="{{ route('shad.create') }}" class="btn btn-primary" aria-label="Add New Shad">Add New Shad</a>
        </div>
    </div>
</div>
...
<div class="card">
    <div class="card-body">
        <div class="table-responsive">
            <table class="table table-striped table-bordered" id="shadTable" aria-label="Shad List">
                <thead>
                    <tr>
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="d-flex align-items-center">
                                <a href="{{ route('shad.show', $property->id) }}" class="btn btn-sm btn-info me-2" aria-label="View Shad Details">View</a>
                                <a href="{{ route('shad.edit', $property->id) }}" class="btn btn-sm btn-warning me-2" aria-label="Edit Shad">Edit</a>
                                <button type="button" class="btn btn-sm btn-danger delete-btn" data-id="{{ $property->id }}" aria-label="Delete Shad">Delete</button>
                            </div>
                        </td>
                    </tr>
                    @endforeach
...
$(document).ready(function() {
    ...
    $(document).on('change', '.status-select', function() {
        var id = $(this).data('id');
        var status = $(this).val();
        var url = '{{ url("admin/shad") }}/' + id + '/update-status';
        var select = $(this);
        
        // Store the previous value in case we need to revert
        var previousValue = select.data('previous-value') || select.val();
