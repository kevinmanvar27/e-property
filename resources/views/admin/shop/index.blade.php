...
<div class="card">
    <div class="card-body">
        <div class="table-responsive">
            <table class="table table-striped table-bordered" id="shopTable">
                <thead>
                    <tr>
                        <th>Sr. No</th>
                        <th>Owner Name</th>
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="d-flex align-items-center">
                                <a href="{{ route('shop.show', $property->id) }}" class="btn btn-sm btn-info me-2">View</a>
                                <a href="{{ route('shop.edit', $property->id) }}" class="btn btn-sm btn-warning me-2">Edit</a>
...
$(document).ready(function() {
    ...
    var table = $('#shopTable').DataTable({
        ...
        initComplete: function() {
            console.log('DataTables initialized for shop table');
        },
        "order": [[ 0, "asc" ]],
        "pageLength": 10,
        "lengthChange": true,
        ...
        if (confirm('Are you sure you want to delete this shop record?')) {
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        toastr.success('Shop record deleted successfully.');
                        // Remove the row from the table
                        table.row(button.closest('tr')).remove().draw();
                    } else {
                        toastr.error('Failed to delete shop record.');
                    }
                },
                error: function(xhr) {
                    toastr.error('An error occurred while deleting the shop record.');
                },
                complete: function() {
    ...
    $(document).on('change', '.status-select', function() {
        var id = $(this).data('id');
        var status = $(this).val();
        var url = '{{ url("admin/shop") }}/' + id + '/update-status';
        var select = $(this);
