    ...
    <div class="ps-3">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb mb-0 p-0">
                ...
                <li class="breadcrumb-item"><a href="{{ route('shop.show', $property->id) }}">Shop Details</a></li>
...
<div class="card">
    <div class="card-header">
        <h5 class="mb-0">Edit Shop Record</h5>
    </div>
    <div class="card-body">
        <form id="shop-form" enctype="multipart/form-data">
            ...
            <div class="d-flex justify-content-end">
                <a href="{{ route('shop.show', $property->id) }}" class="btn btn-secondary me-2">Cancel</a>
                <button type="submit" class="btn btn-primary">Update Shop Record</button>
            </div>
        </form>
    </div>
</div>
...
document.getElementById('shop-form').addEventListener('submit', function(e) {
    ...
    fetch("{{ route('shop.update', $property->id) }}", {
        method: 'POST',
        body: formData,
        headers: {
    ...
    .then(data => {
        if (data.errors) {
            ...
            window.location.href = "{{ route('shop.show', $property->id) }}";
        }
    })
    .catch(error => {
...
function deletePhoto(photoIndex) {
    if (confirm('Are you sure you want to delete this photo?')) {
        fetch(`/admin/shop/{{ $property->id }}/photos/${photoIndex}`, {
            method: 'DELETE',
            headers: {
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
            }
